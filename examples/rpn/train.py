...
from neon.transforms import Rectlin, Softmax, Identity, Logistic, CrossEntropyBinary, SmoothL1Loss
...
conv_layers += [
     Conv((3, 3, 512), init=Gaussian(scale=0.01), bias=Constant(0), activation=Rectlin(),
          padding=1, strides=1),
     b1, Conv((1, 1, 9), init=Gaussian(scale=0.01), bias=Constant(0), activation=Logistic(),
          padding=0, strides=1)
]
...
weights = 1.0/(256)
cost = Multicost(costs=[GeneralizedCostMask(costfunc=CrossEntropyBinary(), weights=weights),
                        GeneralizedCostMask(costfunc=SmoothL1Loss(sigma=3.0), weights=weights)])

# setup optimizer
schedule = Schedule(step_config=[6], change=[0.1])
opt_w = GradientDescentMomentum(0.001 * learning_rate_scale, 0.9, wdecay=0.0005, schedule=schedule)
opt_b = GradientDescentMomentum(0.002 * learning_rate_scale, 0.9, wdecay=0.0005, schedule=schedule)
