...
try:
    # train = ImgMaster(repo_dir=args.data_dir, inner_size=224, set_name='train', subset_pct=10)
...
init1 = Gaussian(scale=0.015)
bias = Constant(val=0.20)
relu = Rectlin()
# drop LR by 4% every 2 epochs
weight_sched = Schedule(range(args.epochs)[2::2], 0.96)
opt_gdm = GradientDescentMomentum(learning_rate=0.045, momentum_coef=0.9, wdecay=0.00004,
...
def main_branch(branch_nodes):
    return [Conv(fshape(7, 64), padding=3, strides=2, **common),
            ...
            Affine(nout=1000, init=init1, activation=Softmax(), bias=bias)]
