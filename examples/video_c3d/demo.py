...
vw = cv2.VideoWriter('output.avi', fourcc, 17, (170, 128))
correct = 0
batch_num = 0
...
for video, label in videos:
    ...
    add_video_mean = video.get() #+ videos.mean
    video_mb_array = add_video_mean.astype(np.uint8)
    for batch_idx in range(be.bsz):
        ...
        top_labels.reverse()
        video_array = video_mb_array[:,batch_idx]
