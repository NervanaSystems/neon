...
def _convert_xml_to_json(input_path, output_path, difficult, img_reshape=None):
    ...
    for file in onlyfiles:
        ...
        trimmed = parse_single_file(join(input_path, file), difficult, img_reshape)
...
def convert_xml_to_json(input_file, output_file, difficult, img_reshape=None):
    trimmed = parse_single_file(input_file, difficult, img_reshape)
    if validate_metadata(trimmed, input_file):
        result = json.dumps(trimmed, sort_keys=True, indent=4, separators=(',', ': '))
        with open(output_file, 'w') as f:
            f.write(result)


def parse_single_file(path, difficult, img_reshape):
    tree = et.parse(path)
    root = tree.getroot()
    ...
    if img_reshape is not None:
        assert len(img_reshape) == 2
        scale = [None, None]
        scale[0] = float(img_reshape[0]) / float(size['height'])
        scale[1] = float(img_reshape[1]) / float(size['width'])
        size['width'] = int(img_reshape[1])
        size['height'] = int(img_reshape[0])
    else:
        scale = [1.0, 1.0]
    ...
    for obj in olist:
        ...
        box = obj['bndbox']
        box['xmax'] = int((float(box['xmax']) - 1) * scale[1])
        box['xmin'] = int((float(box['xmin']) - 1) * scale[1])
        box['ymax'] = int((float(box['ymax']) - 1) * scale[0])
        box['ymin'] = int((float(box['ymin']) - 1) * scale[0])
        if 'part' in obj:
...
if __name__ == "__main__":
    ...
    if parse_file:
        ...
        _convert_xml_to_json(input_path, output_path, args.difficult)
