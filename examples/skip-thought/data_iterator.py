...
class Sentence_Homogenous(NervanaDataIterator):
    ...
    def __init__(self, data_file=None, sent_name=None, text_name=None,
        ...
        self.dev_X_mask = self.be.iobuf(self.max_len, dtype=np.int32)
        self.dev_X_p = self.be.iobuf(self.max_len, dtype=np.int32)  # previous sentence as input
        self.dev_X_p_mask = self.be.iobuf(self.max_len, dtype=np.int32)
        self.dev_X_n = self.be.iobuf(self.max_len, dtype=np.int32)  # next sentence as input
        self.dev_X_n_mask = self.be.iobuf(self.max_len, dtype=np.int32)
    ...
    def next(self):
        """
        Method called by iterator to get a new batch of sentence triplets (source, forward, backward). 
        Sentences are returned in order of increasing length, and source sentences of each batch
        ...
        # get the batch size
        curr_batch_size = np.minimum(self.be.bsz, self.len_curr_counts[self.len_unique[self.len_idx]])
        curr_pos = self.len_indices_pos[self.len_unique[self.len_idx]]

        curr_indices = self.len_indicies[self.len_unique[self.len_idx]][curr_pos:curr_pos+curr_batch_size]
